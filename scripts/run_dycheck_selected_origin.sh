...
OUTPUT_ROOT="$MOSCA_ROOT/output/origin"
RUN_ROOT="$MOSCA_ROOT/logs/mosca_runs"
RUN_ID=$(date +%Y%m%d_%H%M%S)_dycheck_selected_origin
RUN_DIR="$RUN_ROOT/$RUN_ID"
SCENES=(block paper-windmill space-out spin teddy wheel)

mkdir -p "$RUN_DIR" "$OUTPUT_ROOT"
printf '%s\n' "$RUN_DIR" > "$RUN_ROOT/latest_dycheck_selected_run.txt"
...
python scripts/prepare_dycheck_mosca.py apple "${SCENES[@]}" 2>&1 | tee "$RUN_DIR/prepare.log"
